    ...
    if VISUALIZE
        ...
        begin
            ...
            movie = @animate for α in 1:8
                ...
                f = plot(foil.col[1, 1:mid],
                    cps[end][1:mid] ./ (0.5 * flow.Uinf^2),
                    marker = :circle,
                    label = "Bottom")
                plot!(foil.col[1, mid:end],
                    cps[end][mid:end] ./ (0.5 * flow.Uinf^2),
                    marker = :circle,
                    label = "Top")
                plot!(xlabel = "x/c",
                    ylabel = "C_p",
                    legend = :bottomright,
                    title!("C_p vs x/c for α = $(α)"))
                plot!(ylim = (-1.5, 5.0))
                f
            end
