    ...
    for St in Strouhal_values
        for reduced_freq in reduced_freq_values
            for k in ks
                ...
                datas = DataFrame(St = T[],       reduced_freq   = T[],       k = T[], U_inf = T[], t = T[],
                                        σs = Matrix{T}, panel_velocity = Matrix{T}, position =  Matrix{T},
                                        normals =  Matrix{T}, wake_ind_vel = Matrix{T}, 
                                        tangents =  Matrix{T}, μs = T[], pressure = T[])        
                ...
                for i in 1:fp.Ncycles * foil.N
                    ...
                    if i == 1
                        datas = DataFrame(St = [St], reduced_freq = [reduced_freq], k = [k], U_inf=[flow.Uinf], t=[flow.n*flow.Δt],
                                               σs = [foil.σs], panel_vel = [foil.panel_vel], position=[foil.col],
                                               normals = [foil.normals], wake_ind_vel = [foil.wake_ind_vel],
                                               tangents=[foil.tangents], μs = [foil.μs], pressure = [p])
                      
                    else
                        append!(datas, DataFrame(St = [St], reduced_freq = [reduced_freq], k = [k], U_inf=[flow.Uinf],t=[flow.n*flow.Δt],
                                                       σs = [foil.σs], panel_vel = [foil.panel_vel], position=[foil.col],
                                                       normals = [foil.normals], wake_ind_vel = [foil.wake_ind_vel],
                                                       tangents=[foil.tangents], μs = [foil.μs], pressure = [p]))
                    end
