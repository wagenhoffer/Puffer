    ...
    while sum(inside) > 0 && iters < 10
        for i in findall(x -> x == 2, inside)
            ...
            if dest[2, i] >= mid(wake.xy[1, i])
                ...
                tPath = Spline1D(nfoil[1, (N + 1):end],
                    nfoil[2, (N + 1):end] - ms[i] .* nfoil[1, (N + 1):end] .+ bs[i])
                ...
                bPath = Spline1D(nfoil[1, (N + 1):-1:1],
                    nfoil[2, (N + 1):-1:1] - ms[i] .* nfoil[1, (N + 1):end] .+ bs[i])
                if wake.xy[1, i] < dest[1, i]
            ...
            if !isempty(xint)
                ...
                tangents = [-foil.tangents[:, 1:(foil.N ÷ 2)] foil.tangents[:,
                    (foil.N ÷ 2 + 1):end]]
                leg1 = [xint..., yint...] .- wake.xy[:, i] .+
                       flow.δ / 2.0 .* foil.normals[:, whichPanel]
                leg2 = dest[:, i] .- [xint..., yint...] .+
                       flow.δ / 2.0 .* foil.normals[:, whichPanel]
                mag = norm(leg2) * flip
                fin = [xint..., yint...] + mag .* tangents[:, whichPanel] .+
                      flow.δ / 2.0 .* foil.normals[:, whichPanel]
            ...
            elseif sdf(dest[:, i]) < 0.0
                fin = dest[:, i] + nfoil[:, whichPanel] - foil.foil[:, whichPanel] .+
                      flow.δ / 2.0 .* foil.normals[:, whichPanel]
            end
            dest[:, i] = fin
            inside = [sdf(dest[:, i]) for i in 1:size(dest)[2]] .< 0
            inside += map(x -> minimum(nfoil[1, :]) <= x <= maximum(nfoil[1, :]),
                dest[1, :])
            iters += 1
        end
    end
    ...
    movie = @animate for i in 1:(flow.N * 3)
        ...
        while sum(inside) > 0 & iters < 10
            for i in findall(x -> x == 1, inside)
                ...
                #top of foil
                if dest[2, i] >= mid(wake.xy[1, i])
                    tPath = Spline1D(nfoil[1, (foil.N ÷ 2 + 1):end],
                        nfoil[2, (foil.N ÷ 2 + 1):end] -
                        ms[i] .* nfoil[1, (foil.N ÷ 2 + 1):end] .+ bs[i])
                    if wake.xy[1, i] < dest[1, i]
                        xint = filter(x -> wake.xy[1, i] <= x <= dest[1, i], roots(tPath))
                    else
                        xint = filter(x -> wake.xy[1, i] >= x >= dest[1, i], roots(tPath))
                    ...
                    if !isempty(xint)
                        yint = top(xint)
                        whichPanel = findlast(xint .>= nfoil[1, (foil.N ÷ 2 + 1):end]) +
                                     foil.N ÷ 2
                        # print("Intersection $(xint), $(yint)")
                    else
                        whichPanel = findlast(dest[1, i] .>=
                                              nfoil[1, (foil.N ÷ 2 + 1):end]) + foil.N ÷ 2
                    end
                else #bottom of the foil
                    bPath = Spline1D(nfoil[1, (foil.N ÷ 2 + 1):-1:1],
                        nfoil[2, (foil.N ÷ 2 + 1):-1:1] -
                        ms[i] .* nfoil[1, (foil.N ÷ 2 + 1):end] .+ bs[i])
                    if wake.xy[1, i] < dest[1, i]
                        xint = filter(x -> wake.xy[1, i] <= x <= dest[1, i], roots(bPath))
                        flip = -1
                    ...
                    if !isempty(xint)
                        ...
                        whichPanel = findfirst(dest[1, i] .>=
                                               nfoil[1, 1:(foil.N ÷ 2 + 1)]) - 1
                    end
                end
                ...
                if !isempty(xint)
                    ...
                    fin = [xint..., yint...] + mag .* foil.tangents[:, whichPanel] .+
                          flow.δ / 2.0 .* foil.normals[:, whichPanel]
...
begin
    ...
    b = m * stp[1] - stp[2]
    # top and bottom paths for root finding
    tPath = Spline1D(foil.foil[1, (foil.N ÷ 2 + 1):end],
        foil.foil[2, (foil.N ÷ 2 + 1):end] - m .* foil.foil[1, (foil.N ÷ 2 + 1):end] .+ b)
    bPath = Spline1D(foil.foil[1, (foil.N ÷ 2 + 1):-1:1],
        foil.foil[2, (foil.N ÷ 2 + 1):-1:1] - m .* foil.foil[1, (foil.N ÷ 2 + 1):end] .+ b)
    #mid plane
    mid = (top.(foil.foil[1, (foil.N ÷ 2 + 1):end]) +
           bottom(foil.foil[1, (foil.N ÷ 2 + 1):end])) ./ 2.0
    ...
    plot!([foil.foil[1, whichPanel:(whichPan + 1)]],
        [foil.foil[2, whichPanel:(whichPan + 1)]],
        st = :scatter,
        label = "Panel Ends")
    ...
    fin = mag .* foil.tangents[:, whichPanel] + [xint..., yint...] .+
          flow.δ / 2.0 .* foil.normals[:, whichPanel]
...
function compute_signed_distance(x, y)
    ...
    function z_t(x)
        return (0.12 / 0.2) *
               (0.2969 * sqrt(x) - 0.1260 * x - 0.3516 * x^2 + 0.2843 * x^3 - 0.1015 * x^4)
    end
...
begin
    ...
    for i in 1:(flow.Ncycles * flow.N * 0.5)
        time_increment!(flow, foil, wake)
        dest = wake.xy + wake.uv .* flow.Δt
    end
    a = plot_current(foil, wake)
...
function fence!(dest, tree::Cell, foil::Foil, wake::Wake)
    for i in 1:(size(foil.foil)[2] - 1)
        ...
        if length(node.data) > 0
            ...
            sects = map(x -> find_intersection(foil.foil[:, i],
                    foil.foil[:, i + 1],
                    x[1],
                    x[2]),
                [[wake.xy[:, node.data], dest[:, node.data]] for i in 1:length(node.data)])
        ...
        if length(node.data) > 0
            ...
            for (ind, sect) in enumerate(sects)
                if !isnothing(sect)
                    ...
                    tointersetion = (sect + flow.δ * normals[:, i] / 2.0) -
                                    wake.xy[:, node.data[index]]
                    rest = abs(norm(vdist) - norm(tointersetion))
...
function refine_data(r::distanceRefinery, cell::Cell, indices)
    ...
    if !isempty(cell.data)
        for i in 1:length(cell.data)
            if wake.xy[1, cell.data[i]] in intervals[1] &&
               wake.xy[2, cell.data[i]] in intervals[2]
...
plot!(plt, foil.foil[1, :], foil.foil[2, :], marker = :hex, ms = 0.5, label = "")
            ...
            for (ind, sect) in enumerate(sects)
                if !isnothing(sect)
                    ...
                    plot!(plt,
                        ...
                        label = "end")
                    plot!(plt,
                        [foil.foil[:, i] foil.foil[:, i + 1]][1, :],
                        [foil.foil[:, i] foil.foil[:, i + 1]][2, :],
                        label = "")
                    plot!(plt)
                    vdist = wake.xy[:, node.data[ind]] - dest[:, node.data[ind]]
