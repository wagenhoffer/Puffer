...
function refine_data(r::MyRefinery, cell::Cell, indices)
    ...
    hcat([vortices[:,i] for i=1:size(vortices)[2] if vortices[1,i] in intervals[1] && vortices[2,i] in intervals[2]]...)
end
...
intervals = [ClosedInterval(curr.origin[i],curr.origin[i]+curr.widths[i]) for i=1:2]
hcat([vortices[:,i] for i=1:size(vortices)[2] if vortices[1,i] in intervals[1] && vortices[2,i] in intervals[2]]...)
for i = 1:size(vortices)[2]
    all(vortices[j,:] in intervals for j=1:2)
end
filter(pos -> pos[1] in intervals[1] && pos[2] in intervals[2], vortices[1:2, : ])
v_f = hcat([vortices[:,i] for i=1:size(vortices)[2] if vortices[1,i] in intervals[1] && vortices[2,i] in intervals[2]]...)
