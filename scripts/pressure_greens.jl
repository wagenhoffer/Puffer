...
function train_AEs(dataloader, convNN, convNNstate, μAE, μstate, bAE, bstate, mp;ϵ=1e-3)
    ...
    # Train the solution AE - resultant P
    μlosses = []
    kick = false
    for epoch = 1:mp.epochs
        for (_,_,_,_,y) in dataloader        
            y = y./1000.0f0 |> mp.dev
            ls = 0.0
...
function save_state(mp)
    ...
    path = joinpath("data","pressure", μ)
    jldsave(path; μ_state)
    path = joinpath("data","pressure", bdnn)
    jldsave(path; bdnn_state)
    path = joinpath("data","pressure", l)
    jldsave(path; l_state)
    path = joinpath("data","pressure", perf)
...
function make_plots_and_save(mp, dataloader, latentdata, convNN, μAE, B_DNN, L, perfNN, Glosses, μlosses, blosses, convNNlosses)
    ...
    savefig(joinpath(dir_path,"muAE"*join(["$(layer)->" for layer in mp.layers])[1:end-2]*".png"))
...
pinndata = DataLoader((νs=νembs.|>Float32, clts=clts.|>Float32, pos = pos.|>Float32, μs = μs.|>Float32), batchsize=256, shuffle=true)
