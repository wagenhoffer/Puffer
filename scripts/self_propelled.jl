...
function us_relax(;n_loops=25,tolerance=1e-4, Ncycles=4, f=1.0, k=1.0, mass = 1.0, St = 0.3)
    U_init = 1.0        
    for i in 1:n_loops
        ...
        @show (f,k,mass), i, U_init, convergence
    end
...
begin
    ...
    masses = [LinRange{T}(0.1, 2.0, N)]
    Uinits = zeros(length(fs),length(ks),length(masses))
